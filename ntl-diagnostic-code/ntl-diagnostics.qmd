...
if(sum(inter_tf) > 0){
  ...
  saveRDS(data.frame(NULL), file.path(gas_flare_dir, "finaldata", "gas_flare_locations.Rds"))
  saveRDS(data.frame(NULL), file.path(gas_flare_dir, "finaldata", "gas_flare_locations_5km.Rds"))
  saveRDS(data.frame(NULL), file.path(gas_flare_dir, "finaldata", "gas_flare_locations_10km.Rds"))
}
  ...
  for(time_type in c("annual", "monthly", "daily")){
    ...
    for(time_i in time_vec){
      ...
      if(!file.exists(OUT_FILE)){
        ...
        if(bm_product == "VNP46A2"){
          r_path <- file.path(ntl_bm_dir, "rasters", time_type, 
                              paste0(bm_product,"_Gap_Filled_DNB_BRDF-Corrected_NTL_qflag_t",time_raster_i,".tif"))
        } else{
          r_path <- file.path(ntl_bm_dir, "rasters", time_type, 
